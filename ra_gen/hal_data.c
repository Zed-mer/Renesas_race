...
/* Actual period: 0.01 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0xf424,
  .duty_cycle_counts = 0x7a12, .source_div = (timer_source_div_t) 3, .channel = 0, .p_callback = g_timer_agt1_callback,
...
{ .p_ctrl = &g_timer_agt1_ctrl, .p_cfg = &g_timer_agt1_cfg, .p_api = &g_timer_on_agt };
...
#endif
  .ipl = (8),
  .channel = 0, .p_callback = adc0_dma_callback, .p_context = NULL, .activation_source = ELC_EVENT_ADC0_SCAN_END, };
...
/* Actual period: 0.002 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x30d40,
  .duty_cycle_counts = 0x186a0, .source_div = (timer_source_div_t) 0, .channel = 0, .p_callback = NULL,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
