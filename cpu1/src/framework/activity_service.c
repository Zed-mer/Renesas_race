...
typedef struct st_rf_v27_activity_service_context
{
    ...
    rf_v27_activity_round_decision_t pending_decision;
    bool decision_pending;
} rf_v27_activity_service_context_t;

static rf_v27_activity_service_context_t g_activity_service;

volatile rf_v27_activity_service_proof_t g_rf_v27_activity_proof;
volatile uint32_t g_rf_v27_activity_output_generation;

static void rf_v27_activity_service_publish_proof(void)
{
    rf_v27_activity_service_proof_t proof;

    memset(&proof, 0, sizeof(proof));
    proof.magic = RF_V27_ACTIVITY_SERVICE_PROOF_MAGIC;
    proof.version = RF_V27_ACTIVITY_SERVICE_PROOF_VERSION;
    proof.size = (uint16_t)sizeof(proof);
    proof.flags = RF_V27_ACTIVITY_SERVICE_FLAG_ACTIVE;
    ...
    g_rf_v27_activity_output_generation =
        rf_v27_activity_fusion_output_generation(&g_activity_service.fusion);
    g_rf_v27_activity_proof = proof;
}

void rf_v27_activity_service_init(void)
{
    memset(&g_activity_service, 0, sizeof(g_activity_service));
    rf_v27_activity_fusion_init(&g_activity_service.fusion);
    g_activity_service.last_apply_result =
        RF_V27_ACTIVITY_SERVICE_NO_RESULT;
    rf_v27_activity_service_publish_proof();
}

bool rf_v27_activity_service_poll(void)
{
    rf_v13_cpu0_round_message_t message;
    rf_v27_activity_round_decision_t decision;
    rf_v27_apply_result_t result;
    ...
    if (cpu0_epoch_changed)
    {
        ...
        decision.flags |= RF_V27_ROUND_DECISION_CPU0_EPOCH_RESET;
    ...
    decision.flags |= RF_V27_ROUND_DECISION_HAS_MESSAGE;
    ...
    result = rf_v27_activity_fusion_apply_round(
    ...
    switch (result)
    {
        case RF_V27_APPLY_OUTPUT_READY:
            g_activity_service.applied_round_count++;
            output_ready = true;
            decision.flags |= RF_V27_ROUND_DECISION_OUTPUT_VALID;
            ...
            break;
        case RF_V27_APPLY_HELD_INVALID_NO_OUTPUT:
            g_activity_service.held_invalid_round_count++;
            break;
        case RF_V27_APPLY_IGNORED_DUPLICATE:
            g_activity_service.duplicate_round_count++;
            break;
        case RF_V27_APPLY_IGNORED_STALE:
            g_activity_service.stale_round_count++;
            break;
        case RF_V27_APPLY_BAD_MESSAGE:
            g_activity_service.bad_message_count++;
            break;
        case RF_V27_APPLY_BAD_ARGUMENT:
        default:
...
bool rf_v27_activity_service_take_round_decision(
    rf_v27_activity_round_decision_t *decision)
{
...
void rf_v27_activity_service_snapshot(
    rf_v27_activity_service_proof_t *snapshot)
{
    if (snapshot != NULL)
    {
        *snapshot = g_rf_v27_activity_proof;
    }
}
