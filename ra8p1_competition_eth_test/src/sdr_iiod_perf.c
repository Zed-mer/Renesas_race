...
#define SDR_PERF_LOCAL_IP          "192.168.31.20"
#define SDR_PERF_NETMASK           "255.255.255.0"
#define SDR_PERF_GATEWAY           "192.168.31.1"
#define SDR_PERF_PEER_IP           "192.168.31.10"
#define SDR_PERF_IIOD_PORT         30431U
#define SDR_PERF_DURATION_MS       10000U
...
static rt_uint8_t s_io_buffer[SDR_PERF_IO_CHUNK] __attribute__((aligned(4)));
...
static void sdr_perf_update_report(void)
{
    ...
    rt_snprintf((char *)g_sdr_iiod_perf_report,
                ...
                "SDRPERF state=%lu last=%ld ip=%ld/%ld/%ld phy_result=%ld phy_speed=%lu "
                "ping=%ld/%lu connect=%ld version=%s open=%ld bytes=%lu elapsed_ms=%lu "
                "mbps_x1000=%lu reads=%lu chunks=%lu mask=%08lx errors=%lu",
                (unsigned long)result->state,
                (long)result->last_error,
                (long)result->ip_result,
                (long)result->netmask_result,
                (long)result->gateway_result,
                (long)result->phy_result,
                (unsigned long)result->phy_speed,
                (long)result->ping_result,
                (unsigned long)result->ping_time_ms,
                (long)result->connect_result,
                result->version_ok ? (const char *)result->version : "none",
                (long)result->open_result,
...
static int sdr_perf_recv_exact(int socket_fd, void *data, rt_size_t length)
{
    ...
    while (length > 0U)
    {
        int received = recv(socket_fd, cursor, length, 0);
        if (received <= 0)
        {
            return -1;
        }
        cursor += received;
        length -= (rt_size_t)received;
    }
...
static int sdr_perf_recv_line(int socket_fd, char *line, rt_size_t capacity)
{
    ...
    {
        char byte;
        int received = recv(socket_fd, &byte, 1U, 0);
        if (received != 1)
        {
            return -1;
        }
...
static void sdr_perf_thread_entry(void *parameter)
{
    ...
    inet_aton(SDR_PERF_LOCAL_IP, &address);
    ...
    inet_aton(SDR_PERF_GATEWAY, &address);
    ...
    (void)setsockopt(socket_fd, SOL_SOCKET, SO_RCVBUF, &receive_buffer, sizeof(receive_buffer));

    rt_memset(&peer, 0, sizeof(peer));
    peer.sin_family = AF_INET;
    peer.sin_port = htons(SDR_PERF_IIOD_PORT);
    peer.sin_addr.s_addr = inet_addr(SDR_PERF_PEER_IP);
    ...
    rt_strncpy((char *)g_sdr_iiod_perf_result.version,
               line,
               sizeof(g_sdr_iiod_perf_result.version) - 1U);
    ...
    do
    {
        result = sdr_perf_read_stream_block(socket_fd, SDR_PERF_REQUEST_BYTES);
        if (result != 0)
        {
    ...
    } while (sdr_perf_elapsed_ms(start_tick) < SDR_PERF_DURATION_MS);
...
void sdr_iiod_perf_start(void)
{
    ...
    g_sdr_iiod_perf_result.schema_version = 1U;
