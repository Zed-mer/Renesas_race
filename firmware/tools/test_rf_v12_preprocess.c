...
static int test_no_startup_background_and_q10_channels(void)
{
    rf_v12_preprocess_tile_t *tile = calloc(1U, sizeof(*tile));
    ...
    {
        result = test_fail("gain change did not mark legacy state for reset");
        goto done;
    }

    memset(features, 0x55, RF_V12_FEATURE_BYTES);
    rf_v12_preprocess_tile_reset(tile, features);
    ...
    {
        result = test_fail("invalid capture emitted a V31 input tensor");
        goto done;
    }
    info = rf_v12_preprocess_finalize(tile, background, true);
    if ((info.result != RF_V12_PREPROCESS_READY) ||
        (info.background_generation != 0U) ||
        (info.background_became_ready != 0U) ||
        (info.background_reset != 0U))
    {
        result = test_fail("first valid tile was not immediately inference-ready");
        goto done;
    }
    if ((background->calibration_count != 0U) ||
        (background->ready != 0U) ||
        (background->generation != 0U) ||
        (background->reset_pending != 0U))
    {
        result = test_fail("V31 unexpectedly collected startup background state");
        goto done;
    }
         ...
         ++cell)
    {
        ...
        const float expected_raw_c0 = 10.0F * log10f(1140.0F);
        const float expected_c1 = 10.0F * log10f(6000.0F / 1140.0F);
        if (fabsf(tile->c0_db[cell] - expected_raw_c0) > TEST_TOLERANCE_DB)
        {
            result = test_fail("V31 preprocessing mutated raw C0 dBFS");
            goto done;
        }
        if ((features[offset] !=
             rf_v12_preprocess_quantize(0.0F, 0U)) ||
            (features[offset + 1U] !=
        ...
        {
            result = test_fail("V31 Q10 C0/C1/C2/C3 quantization mismatch");
            goto done;
        }
    }
    if (rf_v12_preprocess_finalize(tile, NULL, true).result !=
        RF_V12_PREPROCESS_READY)
    {
        result = test_fail("optional legacy background pointer was still required");
    }
...
int main(void)
{
    if ((test_rne() != 0) ||
        (test_exact_area() != 0) ||
        (test_no_startup_background_and_q10_channels() != 0))
    {
        return 1;
    }
