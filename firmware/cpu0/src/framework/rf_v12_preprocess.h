...
/* Retained for the legacy background-state ABI; V31 does not collect it. */
...
typedef struct st_rf_v12_preprocess_tile
{
    ...
    /* C0 and C1 must remain contiguous for the V31 Q10 feature builder. */
...
/* Legacy per-center storage retained for ABI/source compatibility. V31 uses
 * the current tile's frequency Q10 and never fills calibration or generation. */
...
/* A complete valid tile is immediately encoded for V31. background is
 * optional and is never used to delay inference; the reported generation is 0. */
rf_v12_preprocess_finalize_info_t rf_v12_preprocess_finalize(
    rf_v12_preprocess_tile_t *tile,
    rf_v12_preprocess_background_t *background,
    bool capture_valid);

/* Synthetic compute proof only: encode the same current-tile Q10 tensor. */
