    ...
    bool capture_valid)
{
    ...
    _Static_assert(offsetof(rf_v12_preprocess_tile_t, c1_db) ==
                   offsetof(rf_v12_preprocess_tile_t, c0_db) +
                       sizeof(tile->c0_db),
                   "V31 raw feature channels must remain contiguous");
    rf_v26_build_input(
        (const float (*)[RF_V26_FREQUENCY_BINS][RF_V26_TIME_BINS])
            tile->c0_db,
        (int8_t (*)[RF_V26_TIME_BINS][RF_V26_FEATURE_CHANNELS])
            tile->feature_staging);
