...
struct _lv_display_t {
    ...
    /**
     * A direct-buffer transaction may render several small areas into the
     * off-screen buffer before handing that buffer to the driver.  This is
     * deliberately kept in the display core so a driver never has to make a
     * VSync decision from an application callback.
     */
    uint32_t deferred_active : 1;
    uint32_t deferred_commit : 1;
    uint32_t deferred_sync_done : 1;
    uint32_t deferred_abort : 1;
    uint32_t deferred_resync : 1;
