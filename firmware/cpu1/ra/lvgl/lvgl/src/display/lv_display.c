...
bool lv_display_deferred_begin(lv_display_t * disp)
{
    if(!disp) disp = lv_display_get_default();
    if(!disp || !lv_display_is_double_buffered(disp) ||
       disp->render_mode != LV_DISPLAY_RENDER_MODE_DIRECT) {
        return false;
    }
    if(disp->deferred_active) return true;
    if(disp->deferred_resync) return false;

    disp->deferred_active = 1;
    disp->deferred_commit = 0;
    disp->deferred_abort = 0;
    /* The first deferred refresh still has to synchronize the normal
     * direct-mode dirty areas from the displayed buffer. */
    disp->deferred_sync_done = 0;
    return true;
}

bool lv_display_deferred_commit(lv_display_t * disp)
{
    if(!disp) disp = lv_display_get_default();
    if(!disp || !disp->deferred_active) return false;
    disp->deferred_commit = 1;
    return true;
}

void lv_display_deferred_abort(lv_display_t * disp)
{
    if(!disp) disp = lv_display_get_default();
    if(!disp || !disp->deferred_active) return;
    const bool aborting_flush = disp->flushing != 0;
    const bool buffer_was_touched = disp->deferred_sync_done || aborting_flush;

    disp->deferred_active = 0;
    disp->deferred_commit = 0;
    disp->deferred_sync_done = 0;
    disp->deferred_abort = aborting_flush;

    /* Before the first deferred refresh, only application metadata has
     * changed. The off-screen framebuffer is still coherent, so preserve the
     * normal sync list and avoid an unnecessary full-frame SDRAM resync. */
    if(!buffer_was_touched) return;

    disp->deferred_resync = 1;

    /* A failed transaction must replace, rather than append to, any stale
     * direct-mode sync requests.  Otherwise every later transaction pays for
     * an increasingly large copy and can starve GLCDC scanout. */
    lv_ll_clear(&disp->sync_areas);

    /* The off-screen buffer may contain a partially rendered transaction.
     * Make the next normal direct refresh copy the complete displayed frame
     * before drawing anything else. */
    lv_area_t full = {
        .x1 = 0,
        .y1 = 0,
        .x2 = lv_display_get_horizontal_resolution(disp) - 1,
        .y2 = lv_display_get_vertical_resolution(disp) - 1,
    };
    lv_area_t * sync_area = lv_ll_ins_tail(&disp->sync_areas);
    if(sync_area) *sync_area = full;

    /* Abort can be called from the flush callback itself.  Do not leave LVGL
     * waiting forever for a failed driver submission. */
    if(aborting_flush) lv_display_flush_ready(disp);
}

bool lv_display_deferred_is_active(const lv_display_t * disp)
{
    if(!disp) disp = lv_display_get_default();
    return disp && disp->deferred_active;
}

bool lv_display_deferred_is_commit(const lv_display_t * disp)
{
    if(!disp) disp = lv_display_get_default();
    return disp && disp->deferred_active && disp->deferred_commit;
}

bool lv_display_deferred_is_resyncing(const lv_display_t * disp)
{
    if(!disp) disp = lv_display_get_default();
    return disp && disp->deferred_resync;
}
