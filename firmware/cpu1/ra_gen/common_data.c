...
/* MIPI PHY Macros */
#define MIPI_PHY_CLKSTPT   ((uint16_t)1292.2031250000002)
#define MIPI_PHY_CLKBFHT   ((uint8_t)8 + 1)
#define MIPI_PHY_CLKKPT    ((uint8_t)16 + 4)
#define MIPI_PHY_GOLPBKT   ((uint16_t)28)
...
const mipi_phy_cfg_t g_mipi_phy0_cfg =
		{
				.pll_settings =
						/* Calculated MIPI PHY PLL frequency: 614640000 Hz (error 0.04%) = (24000000 Hz / 3) * 153.66 / 2 */{
								.div = 3 - 1, .pll_div = 1, .mul_int = 153 - 1,
								.mul_frac = 2 /* Value: 0.66 */}, .lp_divisor =
						5 - 1, .p_timing = &g_mipi_phy0_timing, .dsi_mode = (1), };
...
const mipi_dsi_cfg_t g_mipi_dsi0_cfg =
		...
		.sync_pulse = (0), .data_type = MIPI_DSI_VIDEO_DATA_16RGB_PIXEL_STREAM,
				...
				.vertical_active_lines = 600, .vertical_sync_lines = 2,
				.vertical_back_porch = (21 - 2), .vertical_front_porch = (635
						- 600 - 21 - 2), .vertical_sync_polarity =
						(DISPLAY_SIGNAL_POLARITY_LOACTIVE
								!= DISPLAY_SIGNAL_POLARITY_HIACTIVE),

				.horizontal_active_lines = 1024, .horizontal_sync_lines = 24,
				.horizontal_back_porch = (136 - 24), .horizontal_front_porch =
						(1344 - 1024 - 136 - 24), .horizontal_sync_polarity =
						(DISPLAY_SIGNAL_POLARITY_LOACTIVE
								!= DISPLAY_SIGNAL_POLARITY_HIACTIVE),

				.video_mode_delay = 184 /* This value was calculated by FSP. An override is available but not recommended for most users */,

				.hsa_no_lp = ((R_MIPI_DSI_VMSET0R_HSANOLP_Msk
						| R_MIPI_DSI_VMSET0R_HBPNOLP_Msk
						| R_MIPI_DSI_VMSET0R_HFPNOLP_Msk | 0x0)
						& R_MIPI_DSI_VMSET0R_HSANOLP_Msk), .hbp_no_lp =
						((R_MIPI_DSI_VMSET0R_HSANOLP_Msk
								| R_MIPI_DSI_VMSET0R_HBPNOLP_Msk
								| R_MIPI_DSI_VMSET0R_HFPNOLP_Msk | 0x0)
								& R_MIPI_DSI_VMSET0R_HBPNOLP_Msk), .hfp_no_lp =
						((R_MIPI_DSI_VMSET0R_HSANOLP_Msk
								| R_MIPI_DSI_VMSET0R_HBPNOLP_Msk
								| R_MIPI_DSI_VMSET0R_HFPNOLP_Msk | 0x0)
								& R_MIPI_DSI_VMSET0R_HFPNOLP_Msk),

				.num_lanes = 2, .ulps_wakeup_period = 60, .continuous_clock =
						(1),
				...
				.p_extend = &g_mipi_dsi0_extended_cfg, .p_callback =
						mipi_dsi_callback, .p_context = NULL, };
...
const glcdc_extended_cfg_t g_display_extend_cfg = { .tcon_hsync =
		...
		GLCDC_CLK_SRC_INTERNAL, .clock_div_ratio = GLCDC_PANEL_CLK_DIVISOR_6,
		...
		{ .coordinate = { .x = 0, .y = 0 }, .fade_control =
				...
				.output =
						{ .htiming = { .total_cyc = 1344, .display_cyc = 1024,
								.back_porch = 136, .sync_width = 24,
								.sync_polarity =
										DISPLAY_SIGNAL_POLARITY_LOACTIVE },
								.vtiming =
										{ .total_cyc = 635, .display_cyc = 600,
												.back_porch = 21, .sync_width =
														2,
												.sync_polarity =
														DISPLAY_SIGNAL_POLARITY_LOACTIVE },
