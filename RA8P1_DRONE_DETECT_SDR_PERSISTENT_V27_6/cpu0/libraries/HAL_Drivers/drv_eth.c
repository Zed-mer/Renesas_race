...
#ifndef ETH_AUTOMATIC_PAUSE_ENABLE
/* The peer qualification reached a stable ~893 Mbps UDP payload only with
 * automatic PAUSE enabled. Keep an explicit build-time escape hatch for
 * controlled A/B tests without silently shipping the lower-throughput mode. */
#define ETH_AUTOMATIC_PAUSE_ENABLE  1U
#endif
// #define DRV_DEBUG
...
typedef struct eth_diag_counters
{
    uint32_t magic;
    uint32_t open_result;
    uint32_t phy_read_ok;
    uint32_t phy_read_fail;
    uint32_t link_status;
    uint32_t link_process_ok;
    uint32_t link_process_fail;
    ...
    uint32_t mdio_phy_id1[32];
    uint32_t mdio_phy_id2[32];
    uint8_t tx_first_bytes[16];
    uint8_t rx_first_bytes[16];
    uint32_t lwip_tcpip_inpkt_alloc_fail;
    uint32_t lwip_tcpip_inpkt_mbox_fail;
} eth_diag_counters_t;

volatile eth_diag_counters_t g_eth_diag
    __attribute__((section(".ram_nocache"), aligned(32))) =
{
    .magic = 0x45444847U
};

void lwip_perf_diag_tcpip_inpkt_alloc_fail(void)
{
    g_eth_diag.lwip_tcpip_inpkt_alloc_fail++;
}

void lwip_perf_diag_tcpip_inpkt_mbox_fail(void)
{
    g_eth_diag.lwip_tcpip_inpkt_mbox_fail++;
}
...
{
    if ((ETH_AUTOMATIC_PAUSE_ENABLE == 0U) ||
        (ETHER_FLOW_CONTROL_ENABLE != g_ether0_cfg.flow_control))
    {
        return;
    }
...
static uint32_t eth_diag_mdio_read_addr(rmac_phy_instance_ctrl_t *phy_ctrl, uint32_t phy_addr, uint32_t reg_addr)
{
    ...
    mpsm |= R_RMAC0_MPSM_POP_Msk & (2U << R_RMAC0_MPSM_POP_Pos);
    mpsm |= R_RMAC0_MPSM_PDA_Msk & (phy_addr << R_RMAC0_MPSM_PDA_Pos);
    mpsm |= R_RMAC0_MPSM_PRA_Msk & (reg_addr << R_RMAC0_MPSM_PRA_Pos);

    phy_ctrl->p_reg_rmac->MPSM = mpsm;
    phy_ctrl->p_reg_rmac->MPSM_b.PSME = 1;
    FSP_HARDWARE_REGISTER_WAIT(phy_ctrl->p_reg_rmac->MPSM_b.PSME, 0);

    return phy_ctrl->p_reg_rmac->MPSM_b.PRD;
}

static void eth_diag_scan_mdio(void)
{
    rmac_phy_instance_ctrl_t *phy_ctrl = &g_rmac_phy0_ctrl;
    uint32_t addr;

    g_eth_diag.mdio_found_mask = 0U;
    for (addr = 0U; addr < 32U; addr++)
    {
        uint32_t id1 = eth_diag_mdio_read_addr(phy_ctrl, addr, 0x02U);
        uint32_t id2 = eth_diag_mdio_read_addr(phy_ctrl, addr, 0x03U);

        g_eth_diag.mdio_phy_id1[addr] = id1;
        g_eth_diag.mdio_phy_id2[addr] = id2;

        if ((id1 != 0U) && (id1 != 0xFFFFU) &&
            (id2 != 0U) && (id2 != 0xFFFFU))
        {
            g_eth_diag.mdio_found_mask |= (1UL << addr);
        }
    }
}
...
static const phy_port_info_t phy_ports[] = {
    { &g_rmac_phy0_ctrl, 0x01, "PHY0_ET0" }
};
...
static rt_err_t phy_read_status_all(uint8_t *link_status)
{
    ...
    for (i = 0; i < PHY_PORTS_COUNT; i++)
    {
        ...
        if ((res == FSP_SUCCESS) && (phy_data != 0U) && (phy_data != 0xFFFFU))
        {
            g_eth_diag.phy_read_ok++;
...
static rt_err_t phy_get_detailed_status(uint8_t phy_index, uint32_t *basic_status, uint32_t *control_reg)
{
    ...
    if ((res != FSP_SUCCESS) || (*basic_status == 0U) || (*basic_status == 0xFFFFU))
    {
    ...
    if ((res != FSP_SUCCESS) || (*control_reg == 0U) || (*control_reg == 0xFFFFU))
    {
...
static rt_err_t rt_ra_eth_init(void)
{
    ...
    g_eth_diag.link_status = g_link_status;
...
rt_err_t rt_ra_eth_tx(rt_device_t dev, struct pbuf *p)
{
    ...
    g_eth_diag.tx_calls++;
    g_eth_diag.tx_last_len = p->tot_len;
...
#endif
    SMEMCPY((void *)g_eth_diag.tx_first_bytes, buffer, sizeof(g_eth_diag.tx_first_bytes));
#ifdef ETH_RX_DUMP
    if (p)
    {
        LOG_E("******p buf frame *********");
    ...
    g_eth_diag.tx_last_result = (uint32_t)res;
    if (res != FSP_SUCCESS)
    {
        g_eth_diag.tx_fail++;
...
struct pbuf *rt_ra_eth_rx(rt_device_t dev)
{
    ...
    while (1)
    {
        ...
        if (res == FSP_ERR_ETHER_ERROR_NO_DATA)
        {
            g_eth_diag.rx_empty++;
            return NULL;
        }
...
static void phy_linkchange()
{
    ...
    if (res != FSP_SUCCESS)
    {
        g_eth_diag.link_process_fail++;
        LOG_D("R_ETHER_LinkProcess failed!, res = %d", res);
    }
    else
    {
        g_eth_diag.link_process_ok++;
    }
...
{
    /* RMAC link callbacks populate channel but leave legacy status_ecsr unset. */
    uint8_t port_bit = (p_args->channel < 8U) ? (uint8_t)(1U << p_args->channel) : 0U;

    rt_interrupt_enter();

    LOG_D("user_ether0_callback called, event: %d, channel: %lu",
          p_args->event, (unsigned long)p_args->channel);

    switch (p_args->event)
    {
    case ETHER_EVENT_LINK_ON:                          ///< Link up detection event/
        g_eth_diag.irq_link_on++;
        g_link_status |= port_bit;                     ///< status up
        g_link_change |= port_bit;                     ///< change bit set
        g_eth_diag.link_status = g_link_status;
        LOG_I("Interrupt: Link ON detected, status: 0x%02x", g_link_status);
        break;

    case ETHER_EVENT_LINK_OFF:                            ///< Link down detection event
        g_eth_diag.irq_link_off++;
        g_link_status &= (uint8_t)(~port_bit);             ///< status down
        g_link_change |= port_bit;                         ///< change bit set
        g_eth_diag.link_status = g_link_status;
...
static int rt_hw_ra_eth_init(void)
{
    ...
    memset((void *)&g_eth_diag, 0, sizeof(g_eth_diag));
    g_eth_diag.magic = 0x45444847U;
    memset((void *)&g_eth_rmac_diag, 0, sizeof(g_eth_rmac_diag));
    g_eth_rmac_diag.magic = 0x524D4143U;
    g_eth_rmac_diag.version = 1U;
