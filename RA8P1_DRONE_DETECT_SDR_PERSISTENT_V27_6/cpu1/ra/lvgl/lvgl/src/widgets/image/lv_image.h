...
/**
 * Rebind a variable image to another descriptor with identical geometry.
 *
 * Unlike lv_image_set_src(), this function does not invalidate the object or
 * refresh its layout.  It is intended for callers that stage a replacement
 * image into an off-screen framebuffer and explicitly invalidate bounded
 * areas as part of one display transaction.
 *
 * @param obj       pointer to an image object
 * @param src       pointer to an ::lv_image_dsc_t descriptor
 * @return LV_RESULT_OK when the source was rebound, LV_RESULT_INVALID when
 *         either source is not a variable image or its geometry differs
 */
lv_result_t lv_image_rebind_src(lv_obj_t * obj, const void * src);
