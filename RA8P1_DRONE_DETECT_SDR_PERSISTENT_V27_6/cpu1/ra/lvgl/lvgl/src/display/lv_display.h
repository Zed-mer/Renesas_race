...
/**
 * Start a direct/double-buffer transaction.  Flushes generated while the
 * transaction is active are completed to LVGL but do not change the display
 * buffer.  Call lv_display_deferred_commit() before the final invalidation.
 *
 * The API is intended for a single LVGL owner thread.  It does not call the
 * driver or LVGL from an interrupt context.
 */
bool lv_display_deferred_begin(lv_display_t * disp);

/** Mark the next completed direct refresh as the transaction's atomic commit. */
bool lv_display_deferred_commit(lv_display_t * disp);

/** Abort a transaction and force the next normal refresh to resynchronize. */
void lv_display_deferred_abort(lv_display_t * disp);

/** Return true while a direct-buffer transaction is active. */
bool lv_display_deferred_is_active(const lv_display_t * disp);

/** Return true after lv_display_deferred_commit() until the final flush. */
bool lv_display_deferred_is_commit(const lv_display_t * disp);

/**
 * Return true while the framebuffer that was previously visible is being
 * synchronized with a committed direct-mode frame.
 */
bool lv_display_deferred_is_resyncing(const lv_display_t * disp);

/**
 * Copy at most max_bytes from the visible direct buffer to the off-screen
 * buffer.  The LVGL owner should call this from a display-safe scheduling
 * point. Returns the number of bytes copied in this step.
 */
