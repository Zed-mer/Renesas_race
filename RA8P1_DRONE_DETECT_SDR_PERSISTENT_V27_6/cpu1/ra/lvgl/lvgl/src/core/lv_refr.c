...
static uint32_t refr_sync_areas(lv_display_t * disp, uint32_t max_bytes,
                                bool * complete);
...
void lv_display_refr_timer(lv_timer_t * tmr)
{
    ...
    /* The buffer that was visible before a deferred commit can contain an
     * older frame.  Keep invalidations queued until the owner has copied the
     * committed dirty areas into it in bounded, display-safe steps. */
    if(disp_refr->deferred_resync) goto refr_finish;

    lv_refr_join_area();
    if(!disp_refr->deferred_active || !disp_refr->deferred_sync_done) {
        bool sync_complete = false;
        (void)refr_sync_areas(disp_refr, UINT32_MAX, &sync_complete);
        LV_ASSERT(sync_complete);
        if(disp_refr->deferred_active) {
            disp_refr->deferred_sync_done = 1;
        }
    }
    else {
        /* Keep the accumulated transaction areas for post-commit resync.  Do
         * not copy from the old on-screen buffer while this buffer is staged. */
    }
    refr_invalid_areas();
                                ...
                                bool * complete)
{
    ...
    *complete = true;
    ...
    if(disp->render_mode != LV_DISPLAY_RENDER_MODE_DIRECT) return 0;

    /*Do not sync if not double buffered*/
    if(!lv_display_is_double_buffered(disp)) return 0;

    /*Do not sync if no sync areas*/
    if(lv_ll_is_empty(&disp->sync_areas)) return 0;
    if(max_bytes == 0) {
        *complete = false;
        return 0;
    }
    ...
    for(i = 0; i < disp->inv_p; i++) {
        ...
        while(sync_area != NULL) {
            ...
            res_c = lv_area_diff(res, sync_area, &disp->inv_areas[i]);
    ...
    const uint32_t bytes_per_pixel = lv_color_format_get_size(disp->color_format);
    /* Copy complete rows so each owner-scheduled step has a strict SDRAM
     * budget. Leave the unprocessed tail in the list for the next step. */
    sync_area = lv_ll_get_head(&disp->sync_areas);
    while(sync_area != NULL &&
          (max_bytes == UINT32_MAX || copied_bytes < max_bytes)) {
        next_area = lv_ll_get_next(&disp->sync_areas, sync_area);
        /**
        ...
        const uint32_t width = (uint32_t)lv_area_get_width(sync_area);
        const uint32_t height = (uint32_t)lv_area_get_height(sync_area);
        const uint32_t row_bytes = width * bytes_per_pixel;
        uint32_t rows = height;
        if(max_bytes != UINT32_MAX) {
            const uint32_t budget = max_bytes - copied_bytes;
            uint32_t budget_rows = row_bytes != 0 ? budget / row_bytes : height;
            if(budget_rows == 0) {
                if(copied_bytes != 0) break;
                budget_rows = 1;
            }
            if(rows > budget_rows) rows = budget_rows;
        }

        lv_area_t copy_area = *sync_area;
        copy_area.y2 = copy_area.y1 + (int32_t)rows - 1;
#if LV_DRAW_TRANSFORM_USE_MATRIX
        if(lv_display_get_matrix_rotation(disp)) {
            lv_display_rotate_area(disp, &copy_area);
        }
#endif
        lv_draw_buf_copy(off_screen, &copy_area, on_screen, &copy_area);
        if(off_screen2 != on_screen)
            lv_draw_buf_copy(off_screen2, &copy_area, on_screen, &copy_area);
        copied_bytes += rows * row_bytes;
        ...
        if(rows == height) {
            ...
            sync_area->y1 += (int32_t)rows;
            break;
        }
    }

    *complete = lv_ll_is_empty(&disp->sync_areas);
    LV_PROFILER_REFR_END;
    return copied_bytes;
}

uint32_t lv_display_deferred_resync_step(lv_display_t * disp,
                                         uint32_t max_bytes)
{
    if(!disp) disp = lv_display_get_default();
    if(!disp || !disp->deferred_resync) return 0;

    bool complete = false;
    const uint32_t copied = refr_sync_areas(disp, max_bytes, &complete);
    if(complete) {
        disp->deferred_resync = 0;
        if(disp->refr_timer) {
...
static void draw_buf_flush(lv_display_t * disp)
{
    ...
    if(lv_display_is_double_buffered(disp) &&
       ...
       (!disp->deferred_active || disp->deferred_commit) &&
       !disp->deferred_abort) {
        if(disp->buf_act == disp->buf_1) {
        ...
        if(disp->deferred_active && disp->deferred_commit) {
            disp->deferred_active = 0;
            disp->deferred_commit = 0;
            disp->deferred_sync_done = 0;
            disp->deferred_abort = 0;
            disp->deferred_resync = 1;
        }
    }
    else if(disp->deferred_abort) {
        /* The failed transaction deliberately keeps the current scanout
         * buffer.  Consume the one-shot guard after this flush attempt. */
        disp->deferred_abort = 0;
    }
}
